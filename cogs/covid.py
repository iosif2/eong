...
class Covid:
    ...
    async def fetchData(self):
        ...
        data = json.loads(json.dumps(xmltodict.parse(res.content))).get(
            'response').get('body').get('items').get('item')
        self.latest_covid_data['date'] = data[0].get(
            'createDt')[0:10].replace('-', '')
        self.latest_covid_data['new_cases_count'] = int(
            data[0].get('decideCnt')) - int(data[1].get('decideCnt'))
